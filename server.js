...
const ONESIGNAL_API_KEY =
  "os_v2_app_ngufmqneofhjfplr4zzo5x4v7u3da76rbdvuunvbnvhxhju3ynvl6qeyygy5xzpl6vqly7bws4zmi3jwcz3l5zbuwqtpnyeajugkila";
...
function sendPushNotification(title, message) {
  ...
  fetch("https://onesignal.com/api/v1/notifications", {
    ...
    // Thay đoạn body của hàm sendPushNotification bằng đoạn này:
    body: JSON.stringify({
      app_id: ONESIGNAL_APP_ID,
      included_segments: ["Subscribed Users"],
      headings: { en: title },
      contents: { en: message },
      android_sound: "notification", // Tên âm thanh mặc định của android
      ios_sound: "notification", // Tên âm thanh mặc định của ios
    }),
...
io.on("connection", (socket) => {
  ...
  socket.on("register_device", (data) => {
    ...
    if (isNew) ghiLog("THIẾT BỊ", `Phát hiện thiết bị mới: ${data.id}`);
      ...
      ghiLog(
        "THIẾT BỊ",
        `Thiết bị kết nối lại: ${data.ten_hien_thi || data.id}`,
      );
  });
  ...
  socket.on("setup_new_device", (data) => {
    ...
    danhSachThietBi[data.id].loai_thiet_bi =
      data.loai || danhSachThietBi[data.id].loai_thiet_bi;
  ...
  // -------------------------------------------------------
  // NHẬN CÁC SỰ KIỆN TỪ ESP32
  // -------------------------------------------------------

  // 1. CẢNH BÁO NGUY HIỂM (Đỏ)
  ...
  // 2. THÔNG BÁO BÌNH THƯỜNG (Xanh)
  socket.on("trigger_info", (data) => {
    if (!data) return;
    ghiLog("THÔNG BÁO ℹ️", `[${data.type}] ${data.message}`);
    sendPushNotification(`ℹ️ THÔNG BÁO: ${data.type}`, data.message);
    io.emit("alert_broadcast", {
      type: data.type,
      message: data.message,
      level: "info",
